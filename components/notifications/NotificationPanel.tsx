...
export const NotificationPanel: React.FC<NotificationPanelProps> = ({ isOpen, onClose, unreadCount, onUnreadCountChange }) => {
    ...
    const handleMarkAsRead = async (notificationId: string) => {
        ...
        const newCount = notifications.filter(n => !n.leida && n.id !== notificationId).length;
    ...
    return (
        <>
            ...
            {/* Panel */}
            <div className="fixed top-20 right-6 w-full max-w-md bg-white rounded-[2rem] shadow-2xl z-50 animate-in slide-in-from-top-5 duration-300">
                {/* Header */}
                            ...
                            {notifications.map((notif) => (
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        ...
                                        {!notif.leida && (
                                            <div className="w-2 h-2 bg-[#00ADEF] rounded-full flex-shrink-0 mt-2"></div>
                                        )}
                                    </div>
                                </div>
                            ))}
